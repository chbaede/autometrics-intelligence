    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-brand-500/10 text-brand-400 border border-brand-500/20 text-xs font-mono font-semibold">
          <Compass className="w-4 h-4" /> Forward-Looking Guidance & Management Outlook
        </div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
          OEM Financial Guidance & Target Corridors
        </h1>
      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-md space-y-4">
        <div>
          <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
            ...
            Official Management Statements & Target Ranges
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            Verbatim guidance text, publication dates, and key operational assumptions
          </p>
        </div>
        ...
        <div className="space-y-4 pt-2">
          {guidanceList.map((g) => {
            ...
            return (
              <div
                key={g.id}
                className="p-5 bg-slate-950 rounded-xl border border-slate-800 space-y-3"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-900 pb-3">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-100 text-sm">{comp?.name}</span>
                    <span className="text-xs font-mono px-2 py-0.5 rounded bg-slate-900 text-brand-400 border border-slate-800">
                  ...
                  <div className="flex items-center gap-3 text-xs font-mono text-slate-400">
                    ...
                    <span className="px-2 py-0.5 rounded bg-slate-850 text-slate-300 font-semibold">
                      Target: {g.min}% – {g.max}%
                    </span>
                  </div>
                </div>
                ...
                {/* Quoted Statement */}
                <div className="p-3 bg-slate-900 rounded-lg border border-slate-855 text-xs text-slate-300 italic font-mono">
                  "{g.originalText}"
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-1">
                  ...
                  {g.riskNotes && (
                    <div className="space-y-1">
                      ...
                      <p className="text-slate-300">{g.riskNotes}</p>
                    </div>
                  )}
                </div>
                ...
                {src && (
                  <div className="pt-2 border-t border-slate-900 flex items-center justify-between text-xs text-slate-400">
                    ...
                    >
                      <span>View Official Release</span>
