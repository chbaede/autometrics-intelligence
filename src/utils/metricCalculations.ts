  ...
  if (resolvedContext.kind === 'proxy_numerator') {
    ...
    if (!isExceptionApplicable) {
      ...
      const failedProxyChecks = Array.from(new Set([
        ...failedChecks,
        ...(uniqueMismatches.some((m) => m === 'proxyScope' || m === 'targetScope' || m === 'denominatorScope' || m === 'claimScopeMismatch') ? ['scope' as keyof MarginValidationChecks] : []),
        ...(uniqueMismatches.some((m) => m === 'proxyBasis' || m === 'targetBasis' || m === 'denominatorBasis' || m === 'claimAccountingBasisMismatch') ? ['accountingBasis' as keyof MarginValidationChecks] : []),
        ...(uniqueMismatches.some((m) => m === 'period' || m === 'sourcePeriodMismatch' || m === 'missingSourcePeriod' || m === 'inconsistentSourcePeriods' || m === 'invalidPeriodFormat' || m === 'invalidPeriodTypeCombination' || m === 'claimPeriodMismatch') ? ['period' as keyof MarginValidationChecks] : []),
        ...(uniqueMismatches.some((m) => m === 'periodType' || m === 'sourcePeriodTypeMismatch' || m === 'missingSourcePeriodType' || m === 'inconsistentSourcePeriodTypes' || m === 'invalidPeriodTypeCombination' || m === 'invalidPeriodType' || m === 'claimPeriodTypeMismatch') ? ['periodType' as keyof MarginValidationChecks] : []),
